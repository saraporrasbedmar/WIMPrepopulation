...
plt.arrow(x=7.2, y=12, dx=-2.5, dy=0, width=0.7, head_length=0.3,
          facecolor='w')
plt.text(x=7.2, y=15, s='Dark satellites\n(DM)', fontsize=20,
         horizontalalignment='right')

plt.arrow(x=8.9, y=12, dx=5, dy=0, width=0.7, head_length=1,
          facecolor='w')
plt.text(x=8.9, y=15, s='Dwarfs\n(DM + baryons)', fontsize=20,
...
legend11 = plt.legend(handles=handles, handlelength=0.9,
                      fontsize=20,
                      loc=1, bbox_to_anchor=(0.99, 0.45)
                      )
...
plt.ylabel(r'$R_\mathrm{max}$ $\left[\mathrm{kpc}\right]$')
plt.xlabel(r'$V_\mathrm{max}$ '
           r'$\left[\mathrm{km}\,\mathrm{s}^{-1}\right]$')
