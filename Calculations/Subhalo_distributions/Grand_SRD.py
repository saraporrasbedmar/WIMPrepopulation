...
xmin = 2
xmax = 11
linear_tend_dmo = np.polyfit(np.log10(x_med[xmin:xmax] * 1e3),
                             srd_dmo[xmin:xmax], 1)
print('max res dmo')
...
xmin = 5
xmax = 11
linear_tend_hydro = np.polyfit(np.log10(x_med[xmin:xmax] * 1e3),
                               srd_hydro[xmin:xmax], 1)
print('max res hydro')
print(linear_tend_hydro)

plt.plot(xxx, np.log10(xxx) * linear_tend_dmo[0] + linear_tend_dmo[1], '-',
         color='grey', alpha=0.7, label='res_maximum')
plt.plot(xxx, np.log10(xxx) * linear_tend_hydro[0] + linear_tend_hydro[1], '-',
...
srd_hydro_sinVol = np.array(encontrar_SRD_sinVol(Grand_hydro)) / len(
    Grand_hydro)
