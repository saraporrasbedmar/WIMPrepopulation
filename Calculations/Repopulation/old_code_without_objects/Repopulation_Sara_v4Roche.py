...
def SHVF_Grand2012_hydro(V, mm=-3.9313, bb=5.0796):
    # SubHalo Velocity Function - number of subhs defined by their Vmax. Hydro simulation.
    # Grand 2012.07846
    #
    # V - max radial velocity of a bound particle in the subhalo [km/s]
    return 10**bb * V**mm
...
def SHVF_Grand2012_int(V1, V2):
    ...
    if hydro:
        ...
        mm=-3.9557
        bb=5.7313
    return 10**bb / (mm+1) *(V2**(mm+1) - V1**(mm+1))
...
def C_Scatt_DMO(C, sig=.29):
    ...
    return np.array([np.random.lognormal(np.log(C[i]), sig) for i in range(C.size)])
...
#    print(np.shape(Cv))
#    if np.shape(Cv)==():
#        return 1.#newton(def_Cv, 40.0, Cv[0])
#    else:
...
def C200_from_Cv(Cv):
    if np.shape(Cv)==():
        ...
        for i in range(num):
            C200_med[i] = newton(def_Cv, 40.0, Cv[i])#opt.root(def_Cv, 40, args=Cv[i])['x'][0]
...
def Nr_Ntot_resilient_DMO(dist_gc):
    ...
    if resilient:
        ...
        args = [ 0.54343928, -2.17672138]
        return dist_gc**args[0] * 10**args[1]
...
#            print ('     WARNING: no detectability distance cut applied')
        return math.floor(SHVF_Grand2012_int(M1, M2))#integrate.quad(mass_pdf, M1, M2)[0])
...
def cosa_computar ():         
    for it in range(its):
        ...
        if it%1==0:
            print('Hydro:', hydro, ', iteration ', it)
    ...
    #                 repop_Theta, repop_phis)
            ...
            np.savetxt(file6, np.column_stack((repop_J03[Roche_cut], 
...
#            np.savetxt(file_Js_hyd, np.column_stack((repop_Js,
#                                               repop_DistShGc, 
#                                               repop_DistShEarth,
#                                               repop_Vmax, 
#                                               repop_Theta, 
...
#            np.savetxt(file_J03_hyd, np.column_stack((repop_J03,
#                                                   repop_DistShGc, 
#                                                   repop_DistShEarth,
#                                                   repop_Vmax, 
#                                                   repop_Theta, 
#                                                   repop_C)))
    ...
    #                 repop_Theta, repop_phis)
            ...
            Roche_cut_DMO = (R_t(repop_Vmax_DMO, VmaxRmax_Mean_dmo, repop_C_DMO,
                       repop_DistShGc_DMO) < R_s(repop_Vmax_DMO, VmaxRmax_Mean_dmo))
            
            np.savetxt(file5, np.column_stack((repop_Js_DMO[Roche_cut_DMO],
                                               repop_Vmax_DMO[Roche_cut_DMO],
                                               repop_DistShGc_DMO[Roche_cut_DMO])))
            file5.write('\n')
...
#                repop_Js_DMO[bright_Jencia entre DMO y Via Lactea II (hablar con Ale, probab).s_dmo] = 0.
#                bright_Js_dmo = np.where(np.max(repop_Js_DMO)==repop_Js_DMO)[0][0]
#                print('Roche')
...
#            np.savetxt(file_Js_dmo, np.column_stack((repop_Js_DMO,
#                                               repop_DistShGc_DMO,
#                                               repop_DistShEarth_DMO,
#                                               repop_Vmax_DMO, 
#                                               repop_Theta_DMO,
#                                               repop_C_DMO)))
    #        
...
#                print('Roche')
            np.savetxt(file_J03_dmo, np.column_stack((repop_J03_DMO [bright_Js_dmo],
                                               ...
                                               repop_Theta_DMO [bright_Js_dmo],
                                               repop_C_DMO [bright_Js_dmo])))
#            np.savetxt(file_J03_dmo, np.column_stack((repop_J03_DMO,
#                                               repop_DistShGc_DMO,
#                                               repop_DistShEarth_DMO,
#                                               repop_Vmax_DMO, 
#                                               repop_Theta_DMO,
...
path_name = 'holi'+str(resilient)

# Min and max velocities to repopulate
RangeMin, RangeMax = 0.5, 7.
RangeUnit = 'km/s (Vmax)'

VmaxRmax_Mean_dmo = np.array([1.302, -1.444])  # DMO median
VmaxRmax_80pc_dmo = np.array([1.302, -1.245]) # DMO 80 percent

VmaxRmax_Mean_hyd = np.array([1.314, -1.315])  # Hydro median
VmaxRmax_80pc_hyd = np.array([1.314, -1.205])  # Hydro 80 percent
