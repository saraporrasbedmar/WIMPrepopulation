...
minn = np.min((np.min(np.log10(datos_Js_frag_dmo[:,0])), np.min(np.log10(datos_J03_frag_dmo[:,0])),
               np.min(np.log10(datos_Js_frag_hyd[:,0])), np.min(np.log10(datos_J03_frag_hyd[:,0])),
               np.min(np.log10(datos_Js_resi_dmo[:,0])), np.min(np.log10(datos_J03_resi_dmo[:,0])),
               np.min(np.log10(datos_Js_resi_hyd[:,0])), np.min(np.log10(datos_J03_resi_hyd[:,0]))))

maxx = np.max((np.max(np.log10(datos_Js_frag_dmo[:,0])), np.max(np.log10(datos_J03_frag_dmo[:,0])),
               np.max(np.log10(datos_Js_frag_hyd[:,0])), np.max(np.log10(datos_J03_frag_hyd[:,0])),
...
plt.hist(np.log10(datos_Js_frag_dmo[:,0]), log=False, label=r'Frag', color='teal', alpha=0.6,
         bins=bines)#, hatch='x')
plt.hist(np.log10(datos_Js_resi_dmo[:,0]), log=False, label=r'Res', color='k', alpha=0.5,
         bins=bines)#, hatch='o')
...
#plt.annotate(r'J$_{03}$ 95%', (J0395_frag_dmo, 20), rotation=90, color='g', horizontalalignment='right')
...
plt.hist(np.log10(datos_Js_frag_hyd[:,0]), log=False, label=r'Frag', color='yellowgreen', alpha=0.6,
         bins=bines)
plt.hist(np.log10(datos_Js_resi_hyd[:,0]), log=False, label=r'Res', color=darkgreen, alpha=0.6,
         bins=bines)
...
#plt.annotate(r'J$_{03}$ 95%', (Js95_resi_hyd, 20), rotation=90, color='g', horizontalalignment='right')
...
plt.hist(np.log10(datos_J03_frag_dmo[:,0]), log=False, label=r'Frag', color='teal', alpha=0.6,
         bins=bines)
plt.hist(np.log10(datos_J03_resi_dmo[:,0]), log=False, label=r'Res', color='k', alpha=0.5,
         bins=bines)
...
#plt.annotate(r'J$_{03}$ 95%', (J0395_resi_dmo, 20), rotation=90, color='g', horizontalalignment='right')
...
plt.hist(np.log10(datos_J03_frag_hyd[:,0]), log=False, label=r'Frag', color='yellowgreen', alpha=0.6,
         bins=bines)
plt.hist(np.log10(datos_J03_resi_hyd[:,0]), log=False, label=r'Res', color=darkgreen, alpha=0.6,
         bins=bines)
...
fig.text(0.06, 0.5, 'Number of repops', ha='center', va='center', rotation='vertical')
...
plt.legend(handles=legend_elements, handletextpad=0.2,handlelength=1, loc=2, title=r'J$_\mathrm{S}$')
...
plt.legend(handles=legend_elements, handletextpad=0.2,handlelength=1, loc=2, framealpha=0.9, title=r'J$_\mathrm{03}$')
plt.yticks((18,19))
...
plt.ylim(minn-0.1, maxx+0.1)
plt.xlabel(r'$V_\mathrm{max}$ [km s$^{-1}$]', size=22)
plt.legend(handles=legend_elements, handletextpad=0.2,handlelength=1, loc=2, title=r'J$_\mathrm{03}$')
...
plt.plot(constraints_bb_2204[:,0], constraints_bb_2204[:,1]*10**(J03_min95_2204-J0395_frag_dmo), '-k', label='DMO', alpha=0.8)
plt.plot(constraints_bb_2204[:,0], constraints_bb_2204[:,1]*10**(J03_min95_2204-J0395_frag_hyd), '-', label='Hydro', color='limegreen')

plt.plot(constraints_bb_2204[:,0], constraints_bb_2204[:,1]*10**(J03_min95_2204-J0395_resi_dmo), '--k', alpha=0.8)
plt.plot(constraints_bb_2204[:,0], constraints_bb_2204[:,1]*10**(J03_min95_2204-J0395_resi_hyd), '--', color='limegreen')

plt.plot(sigmav_bb_2204[:,0], sigmav_bb_2204[:,1], ':k')
plt.plot(constraints_bb_2204[:,0], constraints_bb_2204[:,1], '--', label='CB+22', alpha=0.5, color='grey')
...
plt.plot(constraints_tau_2204[:,0], constraints_tau_2204[:,1]*10**(J03_min95_2204-J0395_frag_dmo), '-k', label='DMO', alpha=0.8)
plt.plot(constraints_tau_2204[:,0], constraints_tau_2204[:,1]*10**(J03_min95_2204-J0395_frag_hyd), '-', label='Hydro', color='limegreen')

plt.plot(constraints_tau_2204[:,0], constraints_tau_2204[:,1]*10**(J03_min95_2204-J0395_resi_dmo), '--k', alpha=0.8)
plt.plot(constraints_tau_2204[:,0], constraints_tau_2204[:,1]*10**(J03_min95_2204-J0395_resi_hyd), '--', color='limegreen')

plt.plot(sigmav_tau_2204[1:,0], sigmav_tau_2204[1:,1], ':k')
plt.plot(constraints_tau_2204[:,0], constraints_tau_2204[:,1], '--', label='CB+22', alpha=0.5, color='grey')
...
print('%.2f  %.2f  %.2f  %.2f' %(J0395_resi_dmo, J0395_resi_hyd, J0395_frag_dmo, J0395_frag_hyd))
...
minn = np.min((np.min(datos_Js_frag_dmo[:,column]), np.min(datos_J03_frag_dmo[:,column]),
               np.min(datos_Js_frag_hyd[:,column]), np.min(datos_J03_frag_hyd[:,column]),
               np.min(datos_Js_resi_dmo[:,column]), np.min(datos_J03_resi_dmo[:,column]),
               np.min(datos_Js_resi_hyd[:,column]), np.min(datos_J03_resi_hyd[:,column])))

maxx = np.max((np.max(datos_Js_frag_dmo[:,column]), np.max(datos_J03_frag_dmo[:,column]),
               np.max(datos_Js_frag_hyd[:,column]), np.max(datos_J03_frag_hyd[:,column]),
               np.max(datos_Js_resi_dmo[:,column]), np.max(datos_J03_resi_dmo[:,column]),
               np.max(datos_Js_resi_hyd[:,column]), np.max(datos_J03_resi_hyd[:,column])))

#bines = np.linspace(minn, maxx, 25)
bines = np.logspace(np.log10(minn), np.log10(maxx), 40)
locc=2
...
plt.hist((datos_Js_frag_dmo[:,column]), log=False, label=r'Frag', color='teal', alpha=0.6,
         bins=bines)
plt.hist((datos_Js_resi_dmo[:,column]), log=False, label=r'Res', color='k', alpha=0.5,
         bins=bines)
...
plt.hist((datos_Js_frag_hyd[:,column]), log=False, label=r'Frag', color='yellowgreen', alpha=0.6,
         bins=bines)
plt.hist((datos_Js_resi_hyd[:,column]), log=False, label=r'Res', color=darkgreen, alpha=0.6,
         bins=bines)
...
plt.hist((datos_J03_frag_dmo[:,column]), log=False, label=r'Frag', color='teal', alpha=0.6,
         bins=bines)
plt.hist((datos_J03_resi_dmo[:,column]), log=False, label=r'Res', color='k', alpha=0.5,
         bins=bines)
...
plt.hist((datos_J03_frag_hyd[:,column]), log=False, label=r'Frag', color='yellowgreen', alpha=0.6,
         bins=bines)
plt.hist((datos_J03_resi_hyd[:,column]), log=False, label=r'Res', color=darkgreen, alpha=0.6,
         bins=bines)
...
fig.text(0.06, 0.5, 'Number of repops', ha='center', va='center', rotation='vertical')
plt.savefig('aaaaaaaaaaaaaaaaaaa')
...
print(np.shape(np.where(datos_Js_frag_dmo[:,column]<num_min))[1], np.shape(np.where(datos_Js_resi_dmo[:,column]<num_min))[1], '---',
      np.shape(np.where(datos_Js_frag_hyd[:,column]<num_min))[1], np.shape(np.where(datos_Js_resi_hyd[:,column]<num_min))[1])


print(np.shape(np.where(datos_J03_frag_dmo[:,column]<num_min))[1], np.shape(np.where(datos_J03_resi_dmo[:,column]<num_min))[1], '---',
      np.shape(np.where(datos_J03_frag_hyd[:,column]<num_min))[1], np.shape(np.where(datos_J03_resi_hyd[:,column]<num_min))[1])
